...
class TestExportProductAttribute(CatalogManagerTransactionCase):
    ...
    def _bind_value(self):
        return self.create_binding_no_export(
            'prestashop.product.combination.option.value', self.value.id,
            25).with_context(connector_no_export=False)
    ...
    def test_export_product_attribute_onbind(self):
        ...
        self.mock_export_record.delay.assert_called_once_with(
            mock.ANY, 'prestashop.product.combination.option', binding.id,
            priority=20)
    ...
    def test_export_product_attribute_value_onbind(self):
        ...
        binding = self.env['prestashop.product.combination.option.value']. \
            create({
                'backend_id': self.backend_record.id,
        ...
        # check export delayed
        self.mock_export_record.delay.assert_called_once_with(
            mock.ANY, 'prestashop.product.combination.option.value',
            binding.id, priority=20)
    ...
    def test_export_product_attribute_job(self):
        ...
        with recorder.use_cassette(
            ...
            export_record(
                self.conn_session, 'prestashop.product.combination.option',
                binding.id)
            ...
            self.assertEqual('/api/product_options',
            ...
            for field, value in {
                ...
                self.assertEqual(
                    value, ps_option[field]['language']['value'])
    ...
    def test_export_product_attribute_value_job(self):
        ...
        with recorder.use_cassette(
            ...
            export_record(
                self.conn_session,
                'prestashop.product.combination.option.value', binding.id)

            # check request
            self.assertEqual(1, len(cassette.requests))
            request = cassette.requests[0]
            self.assertEqual('POST', request.method)
            self.assertEqual('/api/product_option_values',
                             self.parse_path(request.uri))
            self.assertDictEqual({}, self.parse_qs(request.uri))
            body = self.xmltodict(request.body)
