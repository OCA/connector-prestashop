...
class TestExportProductBrand(CatalogManagerTransactionCase):
    ...
    def setUp(self):
        ...
        self.create_binding_no_export(
            'prestashop.product.brand', parent.id, 2)
    ...
    def _bind_brand(self):
        return self.create_binding_no_export(
            'prestashop.product.brand', self.brand.id, 12).with_context(
                connector_no_export=False)

    @assert_no_job_delayed
    def test_export_product_brand_wizard(self):
        # export from wizard
        wizard = self.env['wiz.prestashop.export.product.brand'].with_context(
            active_ids=[self.brand.id]).create({})
