...
class TestExportProductCategory(CatalogManagerTransactionCase):
    ...
    def setUp(self):
        ...
        self.create_binding_no_export(
            'prestashop.product.category', parent.id, 2)
    ...
    def _bind_category(self):
        return self.create_binding_no_export(
            'prestashop.product.category', self.category.id, 12).with_context(
                connector_no_export=False)

    @assert_no_job_delayed
    def test_export_product_category_wizard(self):
        # export from wizard
        wizard = self.env['wiz.prestashop.export.category'].with_context(
            active_ids=[self.category.id]).create({})
        ...
        self.mock_export_record.delay.assert_called_once_with(
            mock.ANY, 'prestashop.product.category', bindings[0].id,
            priority=20)
    ...
    def test_export_product_category_job(self):
        ...
        with recorder.use_cassette(
            ...
            export_record(
                self.conn_session, 'prestashop.product.category', binding.id)
            ...
            for field, value in {
                ...
                self.assertEqual(
                    value, ps_category[field]['language']['value'])
