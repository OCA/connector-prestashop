...
class TestExportProduct(CatalogManagerTransactionCase):
    ...
    def setUp(self):
        ...
        self.mock_import_record = mock.MagicMock()
        self.patch_import_record = mock.patch(
            'openerp.addons.connector_prestashop.models.binding.common.'
            'import_record',
            new=self.mock_import_record
        )
        self.patch_import_record.start()

    def tearDown(self):
        super(TestExportProduct, self).tearDown()
        self.patch_import_record.stop()
    ...
    def test_export_product_template_wizard_export(self):
        ...
        self.mock_export_record.delay.assert_called_once_with(
            mock.ANY, binding_model, bindings[0].id, priority=20)
    ...
    def test_export_product_template_wizard_active(self):
        ...
        # check export delayed
        self.assertEqual(3, self.mock_export_record.delay.call_count)
        # activate again
        wizard.active_products()
        # check no export delayed
        self.assertEqual(3, self.mock_export_record.delay.call_count)
        ...
        wizard.active_products()
        # check export delayed
        self.assertEqual(4, self.mock_export_record.delay.call_count)
    ...
    def test_export_product_template_wizard_resync(self):
        ...
        # check import done
        self.mock_import_record.assert_called_once_with(
            mock.ANY, 'prestashop.product.template', self.backend_record.id, 8)
    ...
    def test_export_product_template_job(self):
        ...
        with recorder.use_cassette(
            ...
            export_record(
                self.conn_session, 'prestashop.product.template', binding.id)
