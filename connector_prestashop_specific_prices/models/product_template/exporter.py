...
@prestashop(replacing=product.ProductTemplateSpecificPriceExport)
class SpecificProductTemplateRecordExport(
        product.ProductTemplateSpecificPriceExport):
    ...
    def export_pricelist_items(self, erp_record):
        ...
        if pricelist_id:
            item_ids = self.session.env['product.pricelist.item'].search([
                ...
                ('product_tmpl_id', '=', erp_record.odoo_id.id),
                ('product_id', '=', False)]
            )
