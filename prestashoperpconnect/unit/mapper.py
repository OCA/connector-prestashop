...
class SaleOrderMapper(PrestashopImportMapper):
    ...
    def _add_shipping_line(self, map_record, values):
        ...
        if not (amount_incl or amount_excl):
            return values
        line_builder = self.get_connector_unit_for_model(PrestashopShippingLineBuilder)
        if self.backend_record.taxes_included:
            line_builder.price_unit = amount_incl
        else:
            line_builder.price_unit = amount_excl
        if values.get('carrier_id'):
            carrier = self.session.pool['delivery.carrier'].browse(
                self.session.cr, self.session.uid, values['carrier_id'], context=self.session.context)
